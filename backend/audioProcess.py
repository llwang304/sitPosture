...
def process_audio(text, emit_func):
    ...
    def on_message(ws, message):
        try:
            ...
            # else:
            #     audio_segments.append(audio)  # 添加分段音频数据
            #     if status == 2:
            #         ws.close()
            #         full_audio = b''.join(audio_segments)  # 合并分段音频数据
            #         wav_data = pcm_to_wav(full_audio)
            #         print(len(full_audio))
            #         print(type(full_audio))
            #         emit_func(wav_data)  # 发送合并后的 PCM 数据
            # wav版本
