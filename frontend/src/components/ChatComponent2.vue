...
<script setup>
import { ref,onMounted,onUnmounted,reactive,getCurrentInstance,computed,watch} from 'vue';
...
onMounted(() => {
  ...
  socket.on('chat_response', (data) => {
    ...
    messagesHistory.value.push({ role: 'assistant', content: data.content });
...
const sendMessage = async() => {
  if (message.value.trim() !== '') {
    //messages.value.push({ role: 'user', content: message.value });
    messagesHistory.value.push({ role: 'user', content: message.value });
    const conversation_id=conversations.value[activeDialogIndex.value].conversation_id
    console.log("sendmessage 中的conversation_id=",conversation_id)
    //dialogs.value[activeDialogIndex.value].messages.push({ role: 'user', content: message.value });
    proxy.$api.sendUserMessage({ conversation_id:conversation_id,role: 'user', content: message.value });
    console.log({ role: 'user', content: message.value });//添加调试代码
    message.value = '';
    loading.value = true;
...
const fetchConversations = async () => {
  try {
    ...
    // 默认加载最大 conversation_id 的对话
    if (conversations.value.length > 0) {
      //conversations.value.sort((a, b) => b.conversation_id - a.conversation_id);
      //activeConversationId.value = conversations.value[0].conversation_id;
      conversations.value.sort((a, b) => a.conversation_id - b.conversation_id);
      console.log("conversations after sort",conversations.value)
      activeConversationId.value = conversations.value[conversations.value.length - 1].conversation_id;
      console.log("activeDialogIndex",activeDialogIndex)
      activeDialogIndex.value=conversations.value.length-1;
      console.log("activeConversationId",activeConversationId)
      console.log("activeDialogIndex",activeDialogIndex)
      loadMessagesForConversation(activeConversationId.value, 1);  // 加载第一页
    }
    /* if (conversations.value.length > 0) {
      activeConversationId.value = conversations.value[0].conversation_id;
...
const loadMessagesForConversation = async (conversationId, page = 1) => {
  try {
    const phone = JSON.parse(localStorage.getItem("h5_userInfo")).phone;
    const res = await proxy.$api.getMessagesHistory({
      phone,
    ...
    /* if (page === 1) {
      // 首次加载，直接赋值
      messagesHistory.value = newMessages;
    } else {
      // 向上翻页，前插合并历史消息
      messagesHistory.value = [...newMessages, ...messagesHistory.value];
    } */
    messagesHistory.value = res.data.messages;
    console.log("messagesHistory",messagesHistory.value)
...
const currentDialogMessages = computed(() => {
  if (messagesHistory.value.length) {
    return messagesHistory.value;
  }else{
    return [];
  }
});
// 监听 activeDialogIndex 的变化，触发消息加载
watch(activeDialogIndex, (newIndex) => {
  const conversationId = conversations.value[newIndex]?.conversation_id;
  if (conversationId !== undefined) {
    loadMessagesForConversation(conversationId, 1);  // 加载第一页消息
  }
});
</script>
